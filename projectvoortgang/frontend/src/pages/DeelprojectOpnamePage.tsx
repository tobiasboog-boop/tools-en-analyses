...
export default function DeelprojectOpnamePage({ klantnummer }: Props) {
  ...
  // Filter only deelproject regels
  const regels = allRegels?.filter((r) => r.deelproject_jn === 'J') || []
  ...
  if (!regels.length) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 mb-4">Geen deelprojecten gevonden voor deze opname.</p>
        <Link to={`/opname/${key}/paragraaf`} className="text-navy-700 hover:text-navy-500">
          Ga naar paragraaf opname
        </Link>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Deelproject opname</h1>
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase sticky left-0 bg-gray-50">Deelproject</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {regels.map((regel) => (
              <tr key={regel.regel_key} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-gray-500">{regel.projectfase || '-'}</td>
                <td className="px-3 py-2 text-right"><CurrencyDisplay value={regel.calculatie_kosten_inkoop} /></td>
                <td className="px-3 py-2 text-right"><CurrencyDisplay value={regel.geboekte_kosten_inkoop} /></td>
                <td className="px-3 py-2 bg-blue-50">
                  <input type="number" min={0} max={100}
                    ...
                    onChange={(e) => handlePGChange(regel.regel_key, 'percentage_gereed_inkoop', e.target.value)}
                  />
                </td>
                <td className="px-3 py-2 text-right"><CurrencyDisplay value={regel.calculatie_kosten_arbeid_montage} /></td>
                <td className="px-3 py-2 text-right"><CurrencyDisplay value={regel.geboekte_kosten_arbeid_montage} /></td>
                <td className="px-3 py-2 bg-blue-50">
                  <input type="number" min={0} max={100}
                    className="w-16 text-center border border-gray-300 rounded px-1 py-1"
                    value={getPG(regel.regel_key, 'percentage_gereed_arbeid_montage')}
                    onChange={(e) => handlePGChange(regel.regel_key, 'percentage_gereed_arbeid_montage', e.target.value)}
                  />
                </td>
                <td className="px-3 py-2 text-right"><CurrencyDisplay value={regel.calculatie_kosten_arbeid_projectgebonden} /></td>
                <td className="px-3 py-2 text-right"><CurrencyDisplay value={regel.geboekte_kosten_arbeid_projectgebonden} /></td>
                <td className="px-3 py-2 bg-blue-50">
                  <input type="number" min={0} max={100}
                    className="w-16 text-center border border-gray-300 rounded px-1 py-1"
                    value={getPG(regel.regel_key, 'percentage_gereed_arbeid_projectgebonden')}
                    onChange={(e) => handlePGChange(regel.regel_key, 'percentage_gereed_arbeid_projectgebonden', e.target.value)}
                  />
                </td>
              </tr>
            ))}
