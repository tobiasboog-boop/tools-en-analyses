...
def check_password():
    ...
    if not st.session_state.authenticated:
        ...
        st.caption("Coolblue werkbonnen monitoring")
...
# =============================================================================
# CLASSIFICATIE TABELLEN (uit Coolblue SLA contract)
# =============================================================================
    ...
    for label, table in tables.items():
        try:
            ...
            if not sample.empty:
                ...
                id_col = next(
                    (c for c in cols if 'Key' in c or c.lower() in ('id', 'key')),
                    cols[0]
                )
                text_col = next(
                    (c for c in cols
...
def parse_klant_naam(klant_str):
    """Extract klantnaam uit '6897 - Coolblue Winkel Middelburg' format."""
...
def parse_prioriteit_tekst(prio_str):
    """
    Vertaal DWH prioriteit kenmerk naar Coolblue SLA prioriteit.
    Gebaseerd op Syntess codes uit klant 1229:
...
with st.sidebar:
    ...
    sel_opdrachtgever = st.selectbox(
        ...
        index=([i+1 for i, d in enumerate(alle_debiteuren) if "Coolblue" in d] or [0])[0]
    )
...
with tab_export:
    ...
    export_cols = {
        ...
        "onderaannemer_naam": "Welke onderaannemer",
        "contact": "Contact CB",
        ...
        "responstijd_uren": "Responstijd (uren)",
        "hersteltijd_uren": "Hersteltijd (uren)",
...
# === FOOTER ===
st.divider()
st.caption("SLA Tracker v1.0 | Zenith Security (1229) — Coolblue | Notifica B.V.")
