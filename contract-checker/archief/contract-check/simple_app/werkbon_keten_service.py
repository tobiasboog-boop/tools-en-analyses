...
# Create database connection with connection pooling and auto-reconnect
_engine = None

def get_engine():
    """Get database engine with proper pooling settings."""
    global _engine
    if _engine is None:
        db_url = (
...
class WerkbonKetenService:
    ...
    def _ensure_fresh_connection(self):
        ...
        try:
            ...
            # If test fails, try once more with a completely fresh engine
            global _engine
            _engine = None
            self.db = SessionLocal()
